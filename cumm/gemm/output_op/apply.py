...
class ApplyOutputOp(bases.GemmApply):
    ...
    def apply_output_operator(self):
        ...
        out_acc_type = core.array_type(f"typename {self.out_frag_t}::value_type", self.element_per_acc)
        inp_acc_type = core.array_type(f"typename {self.inp_frag_t}::value_type", self.element_per_acc)
    ...
    def apply_output_operator_python(self, output_fragment: ArrayPtr, output_op, aligned_accum_fragment: ArrayPtr, source_fragment: ArrayPtr):
        ...
        output_frag_ptr = output_fragment.change_access_size(self.element_per_acc)
        compute_frag_ptr = aligned_accum_fragment.change_access_size(self.element_per_acc)
    ...
    def apply_output_operator_no_source_python(self, output_fragment: ArrayPtr, output_op, aligned_accum_fragment: ArrayPtr):
        kOutFragCount = output_fragment.length
        kInputFragCount = aligned_accum_fragment.length 
        kOutOpIterations = kOutFragCount // self.element_per_acc
        output_frag_ptr = output_fragment.change_access_size(self.element_per_acc)
        ...
        for i in range(kOutOpIterations):
            output_frag_ptr[i] = output_op.call_op_nosource_python(compute_frag_ptr[i])
