...
class LdMatrix(pccm.ParameterizedClass):
    ...
    def run(self):
        ...
        with code.macro_if_("defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 750)"):
            code.raw("""
            ...
            if self.count == 1:
                ...
                code.raw("reinterpret_cast<int4 &>(D) = make_int4(x, y, z, w);")
    ...
    async def __call__(self, D: ArrayPtr, ptr: ArrayPtr):
        ...
        warp_data = await resource.gather(lane_id, (D, ptr), 0) # type: List[Tuple[ArrayPtr, ArrayPtr]]
        if lane_id == 0:
            Ds = [x[0].change_access_byte_size(2) for x in warp_data]
            # print(Ds)
            smem_ptrs = [x[1] for x in warp_data] 
            for i in range(self.count):
                mat_line_ptrs = smem_ptrs[i * 8: (i + 1) * 8] # type: List[ArrayPtr]
                mat_lines = [p.change_access_byte_size(2) for p in mat_line_ptrs]
                # print(mat_lines)
                for j in range(8):
                    for k in range(4):
                        if self.is_rowmajor:
                            # for tf32 simulation
                            mat_ptr = mat_lines[j].change_access_byte_size(4)[k]
                            Ds[j * 4 + k].change_access_byte_size(4)[i] = mat_ptr
                        else:
                            for l in range(2):
                                mat_ptr = mat_lines[k * 2 + l][j].change_access_byte_size(2)
