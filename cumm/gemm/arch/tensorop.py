...
import pccm 
from cumm.common import TensorView, GemmBasic, TensorViewKernel
...
class MmaSync(pccm.ParameterizedClass):
    ...
    """
    def __init__(self, shape: MetaArray[int], num_threads: int, dtype_a: dtypes.DType, dtype_b: dtypes.DType,
                 dtype_c: dtypes.DType, trans_a: bool, trans_b: bool, trans_c: bool, satfinite: bool = False):
        super().__init__()
    ...
    def call_operator(self):
        ...
        num_32_registers_out = self.dtype_c.bitsize() * self.mn // self.num_threads // 32
        num_32_registers_a = self.dtype_a.bitsize() * self.km // self.num_threads // 32
        num_32_registers_b = self.dtype_b.bitsize() * self.kn // self.num_threads // 32
        out_label = "f" if self.dtype_c == dtypes.float32 else "r"
        cnt = 0
        # register names
        out_register_names = ",".join([f"%{i + cnt}" for i in range(num_32_registers_out)])
        cnt += num_32_registers_out
        a_register_names = ",".join([f"%{i + cnt}" for i in range(num_32_registers_a)])
        cnt += num_32_registers_a
        b_register_names = ",".join([f"%{i + cnt}" for i in range(num_32_registers_b)])
        cnt += num_32_registers_b
        c_register_names = ",".join([f"%{i + cnt}" for i in range(num_32_registers_out)])
        cnt += num_32_registers_out
        code = pccm.FunctionCode()

        # output operands
        out_operands = ", ".join(f"\"={out_label}\"(D[{i}])" for i in range(num_32_registers_out))
        # input operands
        ...
        if num_32_registers_a == 1:
            a_operands = "\"r\"(A)"
            code.raw("unsigned const & A = reinterpret_cast<unsigned const &>(a);")
        else:
            a_operands = ", ".join(f"\"r\"(A[{i}])" for i in range(num_32_registers_a))
            code.raw("unsigned const *A = reinterpret_cast<unsigned const *>(&a);")
        if num_32_registers_b == 1:
            b_operands = "\"r\"(B)"
            code.raw("unsigned const & B = reinterpret_cast<unsigned const &>(b);")
        else:
            b_operands = ", ".join(f"\"r\"(B[{i}])" for i in range(num_32_registers_b))
            code.raw("unsigned const *B = reinterpret_cast<unsigned const *>(&b);")
        ...
        if self.dtype_c == dtypes.float32:
            ...
            code.raw("unsigned const *C = reinterpret_cast<unsigned const *>(&c);")
            code.raw("unsigned *D = reinterpret_cast<unsigned *>(&d);")
        c_operands = ", ".join(f"\"{out_label}\"(C[{i}])" for i in range(num_32_registers_out))
        code.raw(f"""
    ...
    async def __call__(self, d: ArrayPtr, a: ArrayPtr, b: ArrayPtr, c: ArrayPtr):
        ...
        warp_data = await resource.gather(lane_id, (d, a, b, c), 0) # type: List[Tuple[ArrayPtr, ArrayPtr, ArrayPtr,ArrayPtr]]
        ...
        if lane_id == 0:
            ...
            if self.shape == (8, 8, 4):
                ...
                if not self.trans_a:
                    A_qp0 = np.stack(As[w_off[0]:w_off[0] + 4] + As[w_off[0] + 16:w_off[0] + 16 + 4])
                    A_qp1 = np.stack(As[w_off[1]:w_off[1] + 4] + As[w_off[1] + 16:w_off[1] + 16 + 4])
                    A_qp2 = np.stack(As[w_off[2]:w_off[2] + 4] + As[w_off[2] + 16:w_off[2] + 16 + 4])
                    A_qp3 = np.stack(As[w_off[3]:w_off[3] + 4] + As[w_off[3] + 16:w_off[3] + 16 + 4])
                else:
                    A_qp0 = np.concatenate([np.stack(As[w_off[0]:w_off[0] + 4]).T, np.stack(As[w_off[0] + 16:w_off[0] + 16 + 4]).T])
                    A_qp1 = np.concatenate([np.stack(As[w_off[1]:w_off[1] + 4]).T, np.stack(As[w_off[1] + 16:w_off[1] + 16 + 4]).T])
                    A_qp2 = np.concatenate([np.stack(As[w_off[2]:w_off[2] + 4]).T, np.stack(As[w_off[2] + 16:w_off[2] + 16 + 4]).T])
                    A_qp3 = np.concatenate([np.stack(As[w_off[3]:w_off[3] + 4]).T, np.stack(As[w_off[3] + 16:w_off[3] + 16 + 4]).T])
                if not self.trans_b:
                    B_qp0 = np.concatenate([np.stack(Bs[w_off[0]:w_off[0] + 4]), np.stack(Bs[w_off[0] + 16:w_off[0] + 16 + 4])], axis=1)
                    B_qp1 = np.concatenate([np.stack(Bs[w_off[1]:w_off[1] + 4]), np.stack(Bs[w_off[1] + 16:w_off[1] + 16 + 4])], axis=1)
                    B_qp2 = np.concatenate([np.stack(Bs[w_off[2]:w_off[2] + 4]), np.stack(Bs[w_off[2] + 16:w_off[2] + 16 + 4])], axis=1)
                    B_qp3 = np.concatenate([np.stack(Bs[w_off[3]:w_off[3] + 4]), np.stack(Bs[w_off[3] + 16:w_off[3] + 16 + 4])], axis=1)
                else:
                    B_qp0 = np.stack(Bs[w_off[0]:w_off[0] + 4] + Bs[w_off[0] + 16:w_off[0] + 16 + 4]).T
                    B_qp1 = np.stack(Bs[w_off[1]:w_off[1] + 4] + Bs[w_off[1] + 16:w_off[1] + 16 + 4]).T
                    B_qp2 = np.stack(Bs[w_off[2]:w_off[2] + 4] + Bs[w_off[2] + 16:w_off[2] + 16 + 4]).T
                    B_qp3 = np.stack(Bs[w_off[3]:w_off[3] + 4] + Bs[w_off[3] + 16:w_off[3] + 16 + 4]).T
                ...
                if dabc == (dtypes.float16, dtypes.float16, dtypes.float32):
                    for qp_idx in range(4):
                        ...
                        for lane_id_mma in range(w_off[qp_idx], w_off[qp_idx] + 4):
                            for i in range(8):
                                row = (lane_id_mma & 0b1) + (i & 0b10)
                                col = (i & 0b100) + (lane_id_mma & 0b10) + (i & 0b1)
                                C_qp_res[row, col] = Cs[lane_id_mma][i]
                        for lane_id_mma in range(w_off[qp_idx] + 16, w_off[qp_idx] + 16 + 4):
                            for i in range(8):
                    ...
                    C_qp0 = np.stack(Cs[w_off[0]:w_off[0] + 4] + Cs[w_off[0] + 16:w_off[0] + 16 + 4])
                    C_qp1 = np.stack(Cs[w_off[1]:w_off[1] + 4] + Cs[w_off[1] + 16:w_off[1] + 16 + 4])
                    C_qp2 = np.stack(Cs[w_off[2]:w_off[2] + 4] + Cs[w_off[2] + 16:w_off[2] + 16 + 4])
                    C_qp3 = np.stack(Cs[w_off[3]:w_off[3] + 4] + Cs[w_off[3] + 16:w_off[3] + 16 + 4])
                D_qp0_res += C_qp0
                D_qp1_res += C_qp1
                D_qp2_res += C_qp2
                D_qp3_res += C_qp3
                ...
                if dab == (dtypes.float16, dtypes.float16):
                    ...
                    for cid in range(4):
                        for lane_id_mma in range(32):
                            ...
                            col = tid_in_group * 2 + (cid & 0x1)
                            Ds[lane_id_mma][cid] = mma_D[row, col]
                elif dab == (dtypes.int8, dtypes.int8) or dab == (dtypes.uint8, dtypes.uint8):
                ...
                if dab == (dtypes.int8, dtypes.int8) or dab == (dtypes.uint8, dtypes.uint8):
                ...
                mma_D = (mma_A.astype(npdtype_c) @ mma_B.astype(npdtype_c) + mma_C)
                for cid in range(2):
                    for lane_id_mma in range(32):
                        group_id  = lane_id_mma >> 2
                        tid_in_group = lane_id_mma % 4 
