...
class WarpMmaVolta(bases.WarpMma):
    def __init__(self, warp_tile_shape: Tuple[int, int, int], dtype_a: dtypes.DType, dtype_b: dtypes.DType,
                 dtype_c: dtypes.DType, trans_a: bool, trans_b: bool, trans_c: bool):
        super().__init__() 
        self.add_dependency(TensorView)
        self.warp_tile_shape = seq(warp_tile_shape[0], warp_tile_shape[1], warp_tile_shape[2]) 
        ...
        self.fragment_a_count = self.inst_shape[2] * self.mma_iters[0] * self.mma_tile_iters[0]
        self.fragment_b_count = self.inst_shape[2] * self.mma_iters[1] * self.mma_tile_iters[1]
        self.fragment_c_count = self.mma_iters.prod() * self.mma_tile_iters.prod() * 8

        self.fragment_a_t = core.array_type(str(dtype_a), self.fragment_a_count)
        self.fragment_b_t = core.array_type(str(dtype_b), self.fragment_b_count)
        self.fragment_c_t = core.array_type(str(dtype_c), self.fragment_c_count)
        self.mma = tensorop.MmaSync((8, 8, 4), 8, dtype_a, dtype_b, dtype_c, trans_a, trans_b, trans_c)
            ...
            for inner_col in range(self.mma_iters[1]):
                for outer_row in range(self.mma_tile_iters[0]):
                    for inner_row in range(self.mma_iters[0]):
                        ...
                        op_row = inner_row_serp + self.mma_iters[0] * outer_row_serp
                        # op_idx: [kMmaTileIterations[1], kMmaTileIterations[0],
                        # kMmaIterations[1], kMmaIterations[0]]
                        ...
                        await self.mma(ptr_D[op_idx], ptr_A[op_row], ptr_B[op_col], ptr_D[op_idx])
