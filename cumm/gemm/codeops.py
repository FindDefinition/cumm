from typing import List, Generator, Union, Iterable
...
def group_by(key_func: Callable[[_T], _T2], data_iter: Iterable[_T]) -> Dict[_T2, List[_T]]:
...
def unpack(arr_name: str, indices: Iterable[int], op: str=", ", left: str="[", right: str="]"):
    return op.join(f"{arr_name}{left}{i}{right}" for i in indices)

def unpack_str(arr_name: str, indices: Iterable[int], op: str=", ", left: str="_"):
